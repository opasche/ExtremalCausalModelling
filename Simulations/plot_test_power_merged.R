...
for(i in 1:length(sims_codes)){
  ...
  plot(pPmc)
  ggsave(paste(sims_code, "_Pmc_vals_npctc.png", sep=""), plot=pPmc, device="png", path=save_path, width=Wpval, height=Hpval, units="mm", dpi=300)
  ...
  plot(pPmc)
  ggsave(paste(sims_code, "_Pmc_vals_constr_lgpdctc.png", sep=""), plot=pPmc, device="png", path=save_path, width=Wpval, height=Hpval, units="mm", dpi=300)
  ...
  plot(pPmc)
  ggsave(paste(sims_code, "_Pmc_vals_pfcorr_lgpdctc.png", sep=""), plot=pPmc, device="png", path=save_path, width=Wpval, height=Hpval, units="mm", dpi=300)
...
pqq <- ggarrange(p_qq_np.c, p_qq_constr.c, p_qq_pfcorr.c, p_qq_np.ch, p_qq_constr.ch, p_qq_pfcorr.ch, labels="AUTO", ncol=3, nrow=2,
                 common.legend=TRUE,legend="right", vjust=1.5, align="hv") %>%
  annotate_figure(top=text_grob("Uniform QQ-plot for CTC permutation test p-values", face="bold", size=14),
                  left=text_grob("Sample Quantiles", rot = 90), bottom="Theoretical Quantiles")
plot(pqq)
ggsave(paste(substr(sims_codes[1],1,str_length(sims_codes[1])-6), "s_Pmc_QQ.png", sep=""), plot=pqq, device="png", path=save_path, width=250, height=160, units="mm", dpi=300)
